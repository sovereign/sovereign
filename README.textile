...
* Sign in to the ZNC web interface and set things up to your liking. It isn't exposed through the firewall, so you must first set up an SSH tunnel <code>ssh deploy@example.com -L 6643:localhost:6643</code> and then proceed to "$":http://localhost:6643 in your web browser.
